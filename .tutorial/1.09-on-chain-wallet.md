...
Let's begin by reviewing the general architecture for our wallet! Most of it can be found in `src/onchain_wallet/rs`. In this file, you'll find the `OnChainWallet` struct, which encapsulates the functionality we'll need to bridge our on-chain wallet with LDK. Our wallet uses BIP84 descriptors for SegWit v0 addresses, stores data in a SQLite database for persistence, and connects to the blockchain via an Esplora API client.
