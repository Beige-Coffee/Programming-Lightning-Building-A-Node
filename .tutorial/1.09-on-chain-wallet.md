...
- Set `"replaceable": false` to prevent RBF, avoiding potential fund loss in LDK channel funding scenarios.
...
- Use `self.bitcoind_rpc_client.call_method::<FundedTx>` to call `"fundrawtransaction"`.
...
- This matches the expected format for `signrawtransactionwithwallet`’s transaction parameter.
...
- Use `self.bitcoind_rpc_client.call_method::<SignedTx>` to call `"signrawtransactionwithwallet"`.
...
- Use `serde_json::json!(raw_tx.0)` to convert the hex string (accessed via `raw_tx.0`) into a JSON string.
- This matches the expected format for `sendrawtransaction`’s transaction parameter.
...
- Use `self.bitcoind_rpc_client.call_method::<Txid>` to call `"sendrawtransaction"`.
- Pass `&[raw_tx_json]` as the sole argument (the transaction hex).

</details>

<details> <summary> Step 3: Handle the Response </summary>
Await the RPC response, expecting a `Txid`, and discard the result since the method returns `()`.

- `.await` waits for the async RPC call to complete.
- `.unwrap()` assumes the call succeeds (in production, handle errors properly).
- The `Txid` (transaction ID) is received but not used, as the method’s purpose is to broadcast.
