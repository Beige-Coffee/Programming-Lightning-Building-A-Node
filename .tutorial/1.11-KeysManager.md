...
Generate a BIP-32 master private key from the provided 32-byte seed for the Testnet network.
...
```
- `Xpriv::new_master(Network::Testnet, seed)` derives the master extended private key.
- Panics on error (e.g., invalid seed); in production, return an error instead.
- `Network::Testnet` ensures compatibility with testnet chains.
...
```
- `channel_child_index: AtomicUsize::new(0)` initializes a thread-safe counter for channel key derivations.
