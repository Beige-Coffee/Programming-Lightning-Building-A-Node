...
Let's fix that by building data storage capabilities for our Lightning node. To do this, LDK provides a general **Key-Value** storage abstraction that can be connected to a variety of storage solutions such as Google Drive, iCloud, a local disk, or any key-value database. Each **key** will be unique, and it will serve as an identifier for some value (data) we are persisting.
...
Great, we've reviewed *how* we'd like to structure our storage solution, but are we good-to-go? Imagine our Lightning node finds itself in a place where both of the below actions are happening at once:
...
## Atomic Operations
As we saw above, we have to be very careful when building our storage solution. If we find ourselves in a race condition or failed update, our node may lose data, which, in the context of the Lightning Network, can lead to a loss of funds.

To ensure that our data is properly updated, we'll make use of two important tools:
- **Locks**: When dealing with software that utilizes shared resources such as a storge solution, we need to make sure that multiple resources do not try to write to a given file at the same time or that a resource does not attempt to read from a file while it's being written to. To prevent this, we "lock" file. Below are two important types of locking types:
- **Mutual Exclusion (Mutex)**: Only one thread can own a mutex at a time. If another thread attempts to aquire the mutex, it will be blocked until the mutexx is released.
...
We'll start by defining a `FilesystemStore` structure. This will implement LDK's `KVStore` trait, which defines the crucial methods that we must implement so that LDK knows how to use our `FilesystemStore` to properly save our Lightning node's data.
...
impl KVStore for FilesystemStore {
	...
	fn write(
		...
		// Begin atomic write process
		// Create a temporary file path by adding a unique counter
		let mut tmp_file_path = dest_file_path.clone();
		// Generate unique temporary extension using atomic counter
		// Example: "file.1.tmp"
		let tmp_file_ext = format!("{}.tmp", self.tmp_file_counter.fetch_add(1, Ordering::AcqRel));
		tmp_file_path.set_extension(tmp_file_ext);

		// Write data to temporary file and ensure it's synced to disk
		{
				// Create new temporary file
				let mut tmp_file = fs::File::create(&tmp_file_path)?;
				// Write the actual data
				tmp_file.write_all(&buf)?;
				// Force write to disk with fsync
				tmp_file.sync_all()?;
				// tmp_file is dropped here, closing the file
		}

		// Perform the actual atomic rename operation with proper locking
		let res = {
				// Get (or create) a lock for this specific file path
				let inner_lock_ref = {
						// Take the outer lock briefly to access the lock registry
						let mut outer_lock = self.locks.lock().unwrap();
						// Get/create lock for this file and increment its reference count
						Arc::clone(&outer_lock.entry(dest_file_path.clone()).or_default())
				};
				// Take an exclusive write lock on this file
				let _guard = inner_lock_ref.write().unwrap();

				// Atomically rename temporary file to target file
				fs::rename(&tmp_file_path, &dest_file_path)?;
				// Open parent directory for syncing
				let dir_file = fs::OpenOptions::new().read(true).open(&parent_directory)?;
				// Ensure directory entry is synced to disk
				dir_file.sync_all()?;
				Ok(())
				// _guard is dropped here, releasing the write lock
		};
