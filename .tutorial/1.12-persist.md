...
Let's fix that by building data storage capabilities for our Lightning node. To do this, LDK provides a general `KVStore` (**Key-Value**) storage abstraction that can be connected to a variety of storage solutions such as Google Drive, iCloud, a local disk, or any key-value database. Each **key** will be unique, and it will serve as an identifier for some value (data) we are persisting. Under the hood, the `KVStore` implements a variety of functions that will persist channel state data, graph data, storage data, and more.
...
As you can tell, there are a variety of storage solutions available to us when building our node. Below are a few scenarios where we're building an LDK Lightning node. Take a moment and see if you can guess which storage solution of the three (filesystem, SQL, VSS) would be ideal. NOTE: this is an educational exercise, so there isn't necessarily a "correct" answer. When picking you solution, try to imagine "why" that solution would be beneficial in the given circumstance.

<p align="center" style="width: 50%; max-width: 100px;">
	<img src="./tutorial_images/storage_options.png" alt="storage_options" width="80%" height="auto">
</p>

#### Question: Imagine you're node is running on a mobile device and is using its local file system as its storage solution. What happens if you lose or break your phone?
<details>
	<summary>
		Answer
</summary>
