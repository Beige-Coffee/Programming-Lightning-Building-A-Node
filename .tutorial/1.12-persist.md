...
Let's fix that by building data storage capabilities for our Lightning node. To do this, LDK provides a general `KVStore` (**Key-Value**) storage abstraction that can be connected to a variety of storage solutions such as Google Drive, iCloud, a local disk, or any key-value database. Each **key** will be unique, and it will serve as an identifier for some value (data) we are persisting. Under the hood, the `KVStore` implements a variety of functions that will persist channel state data, graph data, storage data, and more. Developers are welcome to build a custom storage solution from scratch, but simply implementing the `KVStore` is sufficient.
