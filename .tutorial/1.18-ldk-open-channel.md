...
- `channel_handshake_limits` sets limits, such as `their_to_self_delay: 2016` (compatible with LND’s maximum `to_self_delay` for counterparty outputs).
...
- `..Default::default()` fills remaining fields with default values, ensuring a complete configuration.
