...
To build our intuition regarding events in LDK, let's review the example of opening up a Lightning channel. For this example, let's imagine that we've build a phone application that interacts with our LDK node. When a use clicks the "Open Channel" button and provides the necessary information to open that channel (funding amount, peer details, etc.), LDK will then send the `open_channel` protocol message to our peer. If our peer response with the `accept_channel` protocol message, LDK will provide us with a `FundingGenerated` event, informing us that we need to create a funding transaction for this channel. Once our wallet creates that transaction, it will hand it back to LDK, and LDK will continue with the channel open steps by sending a `funding_created` protocol message to our peer. 
...
1) **Customization:** LDK is purposefully built to allow for greater customization when building Lightning network applications. For example, in this workshop, we chose to build our on-chain wallet by connecting to Bitcoin Core's wallet functionality via RPC commands. However, we could have just as easily built our wallet using the [Bitcoin Development Kit library](https://bitcoindevkit.org/). Since this on-chain integration is left to us, LDK will let us decide how to create the funding transaction and which UTXOs to select by simply informing us that a transaction needs to be created. 
...
Let's go ahead and start implementing our event handler! To do this, we'll create an asyncronous `handle_ldk_events` function. As you can see, this will have access to some of the  many of the structures that we've reviewed as part of this workshop, such as:
- ChannelManager
- BitcoinClient
- KeysManager
- PeerManager
...
pub async fn handle_ldk_events(
    ...
    match event {
        Event::FundingGenerationReady {
          ...
          // Use on-chain wallet to create raw transaction

          // Use on-chain wallet to add inputs (fund) the transaction

          // Use on-chain wallet to sign transaction
