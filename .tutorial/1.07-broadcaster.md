...
Use the `self.handle` to spawn an asynchronous task for each transaction. This ensures that RPC calls are non-blocking and can run concurrently, improving performance for multiple transactions.
