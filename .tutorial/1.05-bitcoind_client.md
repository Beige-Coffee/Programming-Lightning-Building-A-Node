...
The backbone of Lightning is a fairness protocol. Two parties agree to exchange bitcoin transactions off-chain, but transactions can be published to the chain if a given party needs to enforce the fairness protocol.
...
<p align="center" style="width: 50%; max-width: 300px;">
  <img src="./tutorial_images/on_chain_txs.png" alt="on_chain_txs" width="100%" height="auto">
</p>
...
Let's get to work! We'll start by defining a `struct` in Rust called `BitcoindClient`. In Rust, a struct is a custom data type that groups together related data and functionality - conceptually similar to a `class` in Python. Our `BitcoindClient` will act as the central hub for all interactions with Bitcoin Core. Since Bitcoin Core also implements a wallet, we'll update our `BitcoindClient` to act as our on-chain wallet. By isoloating these functionalities within the `BitcoindClient`, we keep our code modular, maintainable, and aligned with LDK’s architecture.

<p align="center" style="width: 50%; max-width: 300px;">
  <img src="./tutorial_images/bitcoinclient.png" alt="bitcoinclient" width="75%" height="auto">
</p>

If you open `src/bitcoind_client.rs`, you'll see the `BitcoindClient` `struct` is defined for you. It will store important information that is relevant to our Bitcoin Core RPC connection. Most of the fields you see below should be familiar to you, but it's worth decribing two for additional clarity:
...
- `.with_port(port)` sets the port number (e.g., `8332` for Bitcoin mainnet).
...
- `base64::encode` converts the string to Base64 (e.g., `"dXNlcjpwYXNz"`).
...
Create an RPC client using the encoded credentials and HTTP endpoint. This client will handle all communication with the `bitcoind` node.
...
- Unlike some implementations, this version does not make an initial validation call (e.g., `getblockchaininfo`), relying on the `RpcClient::new` method to handle setup.

</details>

<details> <summary> Step 4: Initialize Fee Estimates </summary>
Create a `HashMap` to store fee estimates for different `ConfirmationTarget` variants, using `AtomicU32` for thread-safe fee values.
...
- `MIN_FEERATE` is a constant (assumed to be defined in LDK) representing the minimum acceptable fee rate.
...
- Other fields (`host`, `port`, `rpc_user`, `rpc_password`, `network`, `logger`) are moved or used directly.

</details>

<details> <summary> Step 6: Start Fee Estimate Polling </summary>
Start a background task to periodically poll `bitcoind` for updated fee estimates, using the `poll_for_fee_estimates` method.
...
```
- `poll_for_fee_estimates` is assumed to be a method that spawns a task to update `client.fees`.
