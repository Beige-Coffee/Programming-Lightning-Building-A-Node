...
Before we can get to complex Lightning Network functionality, let's begin by connecting our node to the bitcoin blockchain. After all, we'll start our Lightning journey by mining a funding transaction in the Bitcoin blockchain.
...
#### Question: What are the pros/cons of using a bitcoin node vs esplora as your blockchain source? Think in terms of security, privacy, and ease-of-use.
...
### Security
From a security perspective, running your own node allows you to verify the entire blockchain yourself, without relying on third-parties.

### Privacy
User and application privacy is also enhanced when running your own node, as a third-party is not able to see which transactions you're requesting.

### Ease-Of-Use
Running your own node comes with the technical overhead of additional set-up and maintance. For users who are technically savvy and wish to prioritize privacy and security, then running a node is an ideal solution. On the other hand, Esplora is a plug-and-play solution, which also comes with helpful API calls for application-specific use cases, such as estimating fee rates.
...
That’s where thread safety comes in. Thread safety means our program can handle multiple tasks (like threads or async operations) accessing shared data without causing problems. Imagine if two tasks tried to use the `RpcClient` at once. One may attempt to update some internal state, such as the network graph, while another reads it. Without care, this could lead to a data race, where the data gets corrupted.
...
**You should be good-to-go! Try implementing the `new` constructor in ```src/bitcoind_client.rs```**! Click `Run` at the top of the Replit when you're done to see if the test `tests::programming_lightning::test_03_bitcoind_client_new...` passes.