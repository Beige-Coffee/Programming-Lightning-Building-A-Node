...
use crate::commands::{
	open_channel, send_ln_payment, HTLCStatus, MillisatAmount as SatAmount,
...
mod programming_lightning {
	...
	async fn test_11_open_channel() {
		...
		if let Some((user_channel_id, channel_value_satoshis, config)) = &*final_channel {
			assert_eq!(
				config.channel_handshake_config.their_channel_reserve_proportional_millionths,
				1_000
			);
	...
	fn test_12_send_payment() {
		...
		assert!(final_payments.is_some(), "New payment should be present in Channel Manager");

		// Check if a value is present for a specific key in the FileStore
		let is_value_present = file_store.data.contains_key(&(
			"".to_string(),
			"".to_string(),
			"outbound_payments".to_string(),
		));
		assert!(is_value_present, "New value should be present in the FileStore");
	}

	// Included for potential re-enabling, but commented out as in the provided test suite
	/*
	#[test]
	#[serial]
