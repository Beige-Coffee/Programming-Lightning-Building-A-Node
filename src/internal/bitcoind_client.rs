...
pub struct BitcoindClient {
    pub create_raw_tx: Mutex<Option<Vec<HashMap<String, f64>>>>,
...
impl BitcoindClient {
    ...
    pub async fn create_raw_transaction(&self, outputs: Vec<HashMap<String, f64>>) -> String {
        let mut create_raw_tx = self.create_raw_tx.lock().unwrap();
        *create_raw_tx = Some(outputs);
        "rawtxhex".to_string()
    }

    pub async fn fund_raw_transaction(&self, raw_tx: String) -> String {
        let mut fund_raw_tx = self.fund_raw_tx.lock().unwrap();
        *fund_raw_tx = Some(raw_tx);
        "fundedtxhex".to_string()
    }

    pub async fn sign_raw_transaction_with_wallet(&self, tx_hex: String) -> String {
